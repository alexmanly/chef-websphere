...
def installJDBC ( jdbcname, templateName, driverPath, dsname, dsjndiname, databaseURL, cfname, databasePasswordAlias, databaseUserId, databasePassword, databaseDescription, agedTimeout, connectionTimeout, maxConnections, minConnections, purgePolicy, reapTime, unusedTimeout, stmentCacheSize, jdbcj2eeAttr, jdbcimplclass, jdbcdesc, dsHelper, dsj2eeAttr, stuckTime, stuckTimerTime, stuckThreshold ):
    ...
    if (cmp(updJDBC, "false") == 0):
        if (cmp(templateName, "") != 0):
            #---------------------------------------------------------
            # Get the config id for a JDBC Template
            # Use "lindex" in case there is more than one matching templates -- we
            # just get the first one. 
            #---------------------------------------------------------
            print "---> Getting the JDBCProvider template whose name is "+templateName+" ......"
            template = AdminConfig.listTemplates("JDBCProvider", templateName )
            template = (wsadminToList(template))[1]
            ...
            #---------------------------------------------------------
            print "---> Creating a new JDBCProvider "+jdbcname+" based on "+templateName+" ......"
            for cell in cells:
                jdbcProvider = AdminConfig.createUsingTemplate("JDBCProvider", cell, jdbcAttrs, template )
            #endFor 
            print "     done!"
...
installJDBC("<%= @data[:jdbcName] %>", "<%= @data[:templateName] %>", "<%= @data[:driverPath] %>", 
            "<%= @data[:dataSourceName] %>", "<%= @data[:dsjndiname] %>", "<%= @data[:dsUrl] %>",
            "<%= @data[:cfname] %>","<%= @data[:databasePasswordAlias] %>", "<%= @data[:databaseUserId] %>",
            "<%= @data[:databasePassword] %>", "<%= @data[:databaseDescription] %>", "0", "1900", "50", "5",
            "EntirePool", "180", "1800", "10", "", "<%= @data[:driverClass] %>", "<%= @data[:jdbcDescription] %>",
            "<%= @data[:dsHelper] %>", "", "0", "0", "0" )
